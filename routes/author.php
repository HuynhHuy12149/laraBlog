...
Route::prefix('author')->name('author.')->group(function(){
    // route chưa đăng nhập truy cập được sử dụng middleware này
    Route::middleware(['guest:web'])->group(function(){
        Route::view('/login','back.pages.auth.login')->name('login');
        Route::view('/forgot-password','back.pages.auth.forgot')->name('forgot-password');
        Route::get('/password/reset/{token}',[AuthorController::class,'ResetForm'])->name('reset-form');
    });
    // route khong ap dung bơi middle nào su dung phuong thuc get lấy data xử lý bởi index
    Route::middleware(['auth:web'])->group(function(){
        Route::get('/home',[AuthorController::class,'index'])->name('home');
        Route::get('/author/login', [AuthorController::class, 'showLoginForm'])->name('author.login');
